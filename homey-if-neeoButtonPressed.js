...
neeoButtonPressedDevice.registerAutocompleteListener((query, args) => {
  return homeyAutocomplete.devices(query, args);
});

let neeoButtonPressedCapability = neeoButtonPressed.getArgument("capabilitie");
neeoButtonPressedCapability.registerAutocompleteListener((query, args) => {
  return homeyAutocomplete.capabilities(query, args, "button");
});
