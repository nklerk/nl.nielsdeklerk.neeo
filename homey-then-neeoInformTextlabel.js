...
neeoInformTextlabelDevice.registerAutocompleteListener((query, args) => {
  return homeyAutocomplete.devices(query, args);
});

let neeoInformTextlabelCapability = neeoInformTextlabel.getArgument("capabilitie");
neeoInformTextlabelCapability.registerAutocompleteListener((query, args) => {
  return homeyAutocomplete.capabilities(query, args, "custom");
});
