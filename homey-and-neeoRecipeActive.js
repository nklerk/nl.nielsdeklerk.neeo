...
neeoRecipeActiveRoom.registerAutocompleteListener((query, args) => {
  return homeyAutocomplete.rooms(query, args);
});

let neeoRecipeActiveRecipe = neeoRecipeActive.getArgument("recipe");
neeoRecipeActiveRecipe.registerAutocompleteListener((query, args) => {
  return null, homeyAutocomplete.recepies(query, args, "launch");
});
